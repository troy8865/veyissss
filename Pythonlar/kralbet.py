...
# kralbet.m3u dosyasına yaz
with open("kralbet.m3u", "w", encoding="utf-8") as f:
    f.write("#EXTM3U\n\n")
    for idx, ch in enumerate(channels, 1):
        f.write(f'#EXTINF:-1 tvg-id="Kralbet{idx}" tvg-name="{ch["name"]}" tvg-logo="{ch["logo"]}" group-title="KRALBET",{ch["name"]}\n')
        f.write(f"#EXTVLCOPT:http-user-agent={headers['User-Agent']}\n")
        f.write(f"#EXTVLCOPT:http-referrer={headers['Referer']}\n")
        f.write(ch["link"] + "\n\n")

print("kralbet.m3u dosyası başarıyla oluşturuldu.")
